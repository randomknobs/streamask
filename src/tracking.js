...
export function createLandmarkSmoother(){
  ...
  return {
    // Принудительный холодный старт на следующий update() — тот же путь,
    // что и при самом первом кадре (smoothed===null ниже): маска садится
    // на сырые лендмарки без единого кадра лерпа. Нужен при возврате лица
    // после потери трекинга (main.js) — иначе "куда сейчас нужно доехать"
    // считалось бы от позиции, замороженной ещё до пропажи, и лицо видимо
    // подъезжало бы к новому месту несколько кадров вместо мгновенной посадки.
    reset(){ smoothed = null; },
    update(rawLms){
      if (!smoothed){
